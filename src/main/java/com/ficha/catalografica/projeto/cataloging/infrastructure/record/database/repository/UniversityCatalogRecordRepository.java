package com.ficha.catalografica.projeto.cataloging.infrastructure.record.database.repository;

import java.util.List;

import org.springframework.data.jpa.repository.JpaRepository;

import com.ficha.catalografica.projeto.cataloging.infrastructure.record.database.entity.UniversityCatalogRecordEntity;

public interface UniversityCatalogRecordRepository extends JpaRepository<UniversityCatalogRecordEntity, String> {

  List<UniversityCatalogRecordEntity> findAllByCreatorId(String creatorId);
}
